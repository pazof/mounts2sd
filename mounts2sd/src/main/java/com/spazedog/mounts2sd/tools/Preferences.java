			...
			synchronized (mInstanceLock) {
				...
				if (root.isConnected() && (force || !isLoaded())) {
					...
					if ( (internalCheck = root.busybox().exists()) || environment_multiple_binaries()) {
                        ...
                        boolean cmdalvik_veto_config = root.property().exists("dalvik.vm.dexopt-data-only") &&
                                "1".equals(root.property().get("dalvik.vm.dexopt-data-only"));
						support_directory_cmdalvik(root.file("/cache/dalvik-cache").isDirectory() && !cmdalvik_veto_config);
